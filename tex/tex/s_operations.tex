...
\label{alg:subexists}
\begin{algorithmic}[1]
\Function{\textsc{submsetExistence}}{$\mathcal{M}$, $m$}
\State $currentNode \gets \mathcal{M}$
...
\If {\textsc{submsetExistence}($c_i,$ $m[2:]$)}
\State \Return True
\EndIf
\EndIf
\EndFor
...
\begin{algorithmic}[1]
\Function{\textsc{supermsetExistence}}{$\mathcal{M}$, $m$}
\State $currentNode \gets \mathcal{M}$
\If {$currentNode$ is \emph{accepting} }
\State \Return True
\EndIf
\For {$i=m[1]$ to $n-1$}
\If {child $c_i$ of $currentNode$ is not \emph{Null}}
\If {\textsc{supermsetExistence}($c_i,$ $m[2:]$)}
\State \Return True
\EndIf
\EndIf
\EndFor
