...
We now focus our attention to the distribution of $\xi_{ij}$.
...
additional space must be provided for multisets storage. 
...
%level. 
As we increase the parameter $p$, the number of sets we need to store (i.e. number of leaves) increases linearly. The figure~\ref{f:exp-nodes} shows that the additional overhead space that our data-structure uses does not increase as the tree is becoming more and more dense.


\begin{figure}[h!]
\center
\includegraphics[width=.4\textwidth, keepaspectratio]{exp-nodes-on-probab.pdf}
\caption{Size of a tree ($\mathbb{E}(|\mathcal{M}|)$) compared to the number of stored sets ($\mathbb{E}(|M|)$), as the probability parameter $p$ increases.
Parameters 
...
\begin{figure}[h!]
\center
\includegraphics[width=.4\textwidth, keepaspectratio]{ratio-exp-nodes-and-msets-on-prob1.pdf}
...
Analyzing the graph on figure~\ref{f:ratio-exp-msets} we can safely 
...
We will see, in our view, great results throughout various experiments, even when compared with state-of-the-art approaches like Inverted Index.

It was very frustrating to us that, despite those indications, it seems that this structure is very difficult to analyze. 
Even after restricting the analysis to the (usually simple) case when the input data is uniformly distributed, and after utilising reliable probabilistic tools, our approach unfortunately did not give as clear results as we hoped for. 
We would be very keen to learn of the approach or technique which would shed more insight into the mathematical analysis of our data-structure and its algorithms. 
